...
export default function OverviewCards({ stats }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      <Card animate tilt animateDelay={0} className="px-4 py-3 flex flex-col gap-1">
        <span className="text-text-muted text-sm uppercase font-semibold">Total Requests</span>
        <span className="text-2xl font-bold">{fmt(stats.totalRequests)}</span>
      </Card>
      <Card animate tilt animateDelay={50} className="px-4 py-3 flex flex-col gap-1">
        <span className="text-text-muted text-sm uppercase font-semibold">Total Input Tokens</span>
        <span className="text-2xl font-bold text-primary">{fmt(stats.totalPromptTokens)}</span>
      </Card>
      <Card animate tilt animateDelay={100} className="px-4 py-3 flex flex-col gap-1">
        <span className="text-text-muted text-sm uppercase font-semibold">Output Tokens</span>
        <span className="text-2xl font-bold text-success">{fmt(stats.totalCompletionTokens)}</span>
      </Card>
      <Card animate tilt animateDelay={150} className="px-4 py-3 flex flex-col gap-1">
